...
| CDPA e Cláusulas Contratuais-Padrão brasileiras | Confirmação do owner sobre a natureza da conta; termos padrão do Google Cloud. | O acordo da Ella incorpora o Cloud Data Processing Addendum e as SCCs brasileiras antes de qualquer entrevista real com transferência internacional. | **VERIFICADO, 2026-08-05:** owner confirmou que a conta é padrão/self-serve (sem contrato Google negociado). Nos termos padrão, o [Cloud Data Processing Addendum](https://cloud.google.com/terms/data-processing-addendum) é incorporado automaticamente e incorpora as [SCCs brasileiras (BR C2P)](https://cloud.google.com/sccs/br-c2p) quando a LGPD se aplica. Mecanismo de transferência internacional: coberto. |
| Região e endpoint do Vertex | Código-fonte: `backend/llm/gemini.py` (`aiplatform.init(project=..., location=settings.llm_location)`). | `southamerica-east1` quando disponível; jamais endpoint `global` para este fluxo. Se outra região for usada, registrar a transferência e a justificativa. | **CONFIGURADO, 2026-08-06:** `LLM_LOCATION` é explícito e o padrão permanece `us-central1`. Não é endpoint `global`, mas também não é São Paulo. Transferência internacional existe e depende do mecanismo CDPA/SCCs (linha acima). **Gate do owner/provider:** testar disponibilidade do `gemini-2.5-flash` em `southamerica-east1` antes de alterar a região. |
...
1. **CDPA + SCCs brasileiras** — ✅ **RESOLVIDO, 2026-08-05:** conta padrão confirmada pelo owner; CDPA + SCCs brasileiras aplicam-se por padrão (ver linha na tabela acima).
2. **Opt-out de monitoramento de abuso** — `SOLICITADO` (2026-08-03), decisão do Google em ~2 semanas. **DECISÃO DO OWNER, 2026-08-05:** estado "solicitado-pendente" **aceito** para as entrevistas internas da semana 1. Racional registrado: o logging de abuso só ocorre em prompts *sinalizados* por classificadores; o cache — retenção padrão — já está desativado; candidatos são avisados; conteúdo coberto pelo CDPA. Registrar a aprovação do Google quando o e-mail chegar.

**Resultado: a barreira de privacidade para a primeira entrevista real está LIBERADA** (todos os itens verificados, documentados ou aceitos por decisão datada do owner).
