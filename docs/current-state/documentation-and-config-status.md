...
Phase 0B is still incomplete, but billing and the inactive service/data/provider boundary are now configured. The isolated project has private empty Firestore/GCS resources, an empty secret container, a disabled least-privilege runtime identity, verified-disabled STT data logging, verified-disabled Vertex cache, and a BRL 250 monthly project budget. Source preservation, branch/environment protection before deployment restoration, and panel re-review remain. Phase 1 stays blocked. See `docs/current-state/phase-0b-containment-evidence.md`.
