...
2. **Windows recruiter + machine named** — ⏳ STILL OPEN, clarified: the question is *which specific person in the first launch cohort works on a Windows machine*, so the W1 spike runs on their real hardware. If nobody in the cohort uses Windows, the §5 gate defaults to macOS-first at zero cost. *Needed by W1.*
