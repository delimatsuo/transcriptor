...
6. **Notarize and staple**: run `xcrun notarytool submit "dist/${APP_NAME}.dmg" --keychain-profile "$NOTARY_PROFILE" --wait` under guarded status/output capture so `set -e` cannot skip failure handling. Require a non-empty returned submission ID and an explicit accepted status before any stapling. If the status is `Invalid`, call `xcrun notarytool log "$SUBMISSION_ID" --keychain-profile "$NOTARY_PROFILE"` exactly once, print the diagnostic, and exit non-zero. Any missing/unparseable ID or status, submit transport failure, or non-accepted status must fail closed. Only after acceptance may the script run `xcrun stapler staple "dist/${APP_NAME}.dmg"` and `xcrun stapler validate "dist/${APP_NAME}.dmg"`.
...
5. An invalid synthetic submission returns a fixed fake ID and `Invalid` status. Assert that `notarytool log` receives that exact ID and the selected profile exactly once, the diagnostic is printed, the script exits non-zero, and no stapling, validation, assessment, hashing/final-success summary, or later protected call occurs.
6. Table-drive non-zero failures at synthetic packaging, executable/bundle signing and verification, DMG creation/signing, submission transport, stapling, validation, and assessment. Each case must exit non-zero, make no later protected call, and emit no final-success summary.
7. The test fails if any protected command or packaging script resolves outside the shim/temp tree, if any unexpected protected shim is invoked, if the real repository's `dist/` changes, if any failure path continues past its protected edge, or if the exact temporary tree is not removed.
